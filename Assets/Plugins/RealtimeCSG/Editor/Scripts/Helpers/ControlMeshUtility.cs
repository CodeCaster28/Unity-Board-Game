...
//				var polygon_bounds	= polygonBounds[s];
		...
		public static bool RebuildShapeFrom(CSGBrush brush, ControlMesh srcControlMesh, Shape srcShape)
		{
			...
			brush.Shape					= triangulatedShape;
			brush.ControlMesh			= triangulatedControlMesh;
		...
		public static bool DeleteSelectedPoints(CSGBrush[] brushes, ControlMeshState[] controlMeshStates)
		{
			...
			for (var t = brushes.Length - 1; t >= 0; t--)
			{
				...
				var controlPoints = new List<short>();
				for (var p = meshState.PointSelectState.Length - 1; p >= 0; p--)
				{
					if ((meshState.PointSelectState[p] & SelectState.Selected) != SelectState.Selected)
						continue;
					controlPoints.Add((short)p);
				}
		...
		public static void Flip(CSGBrush[] brushes, Matrix4x4 flipMatrix, string undoDescription = "Flip brushes")
		{
			...
			for (var t = 0; t < brushes.Length; t++)
			{
				...
				brush.ControlMesh = CreateFromShape(shape);
