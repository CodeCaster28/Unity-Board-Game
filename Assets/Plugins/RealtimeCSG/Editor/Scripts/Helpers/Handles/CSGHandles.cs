...
namespace RealtimeCSG.Helpers
{
	public sealed partial class CSGHandles
	{
		...
		public static void CubeHandleCap(int controlID, Vector3 position, Quaternion rotation, float size, EventType eventType)
		{
			...
			Handles.CubeHandleCap(controlID, position, rotation, size * .2f, eventType);
#else
			switch (eventType)
			{
		...
		{
#if UNITY_5_6_OR_NEWER
			Handles.ArrowHandleCap(controlID, position, rotation, size * .2f, eventType);
#else
			switch (eventType)
			{
				case EventType.Layout:
		...
		{
#if UNITY_5_6_OR_NEWER
			Handles.ConeHandleCap(controlID, position, rotation, size * .2f, eventType);
#else
			switch (eventType)
			{
				case (EventType.Layout):
